...
const UrlPage = ({ longUrl, baseUrl, url }) => {
	...
	if (longUrl === null) {
		return (<p>Short-URL: "{baseUrl}/{url}" is not in use, <a href={baseUrl}>click here to use this url</a> </p>)
	}
	...
	return(
		<div>
			...
			<main>
				<p>Redirecting to: <a href={longUrl} target='_blank'>{longUrl}</a></p>
			</main>
		</div>
	)
}
...
export async function getServerSideProps(context) {
	...
	const baseUrl = process.env.NODE_ENV === 'development' ? 'http://localhost:3000' : 'https://in.now.sh'
