...
export default function Youtube() {
	...
	const isValidYoutubeUrl = (url) => {
		const regex = /^https:\/\/(www\.youtube\.com\/watch\?v=|youtu.be\/)\w{8,13}$/  // I found - every time \w{11} in long-urls
		return regex.test(url)
	}
		...
		<div>
			...
			<main>
				<form>
					...
					<button className='create-btn' type='button' onClick={createShortUrl}>Create</button>
				</form>
				<p>{message0}</p>

				<div className='creation-result' style={shortUrl.length > 0 ? {} : { display: 'none' }}>
					<p>
						Youtube Short-URL: <span style={{ marginLeft: '7px' }}>{shortUrl}</span>
